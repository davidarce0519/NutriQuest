...
export const FoodCatalogScreen = () => {
  ...
  const detailTiming =
    !selectedFood?.energyLevel ? null :
      selectedFood.energyLevel >= 4 ? { icon: '☀️', text: 'Antes de clases o estudio intenso' } :
        selectedFood.energyLevel >= 2 ? { icon: '🌤', text: 'A media tarde o entre clases' } :
          { icon: '🌙', text: 'En la noche o como snack ligero' };

  const renderFood = ({ item }: { item: Food }) => (
    <TouchableOpacity
      style={[s.foodCard, { backgroundColor: cardBg, borderColor: border }, !item.isActive && { opacity: 0.55 }]}
      ...
      <View style={s.cardChipsWrap}>
        ...
        {!!item.energyLevel && (
          <View style={[s.cardChip, { backgroundColor: ENERGY_COLORS[item.energyLevel] + '22' }]}>
            <Text style={[s.cardChipText, { color: ENERGY_COLORS[item.energyLevel] }]}>⚡{item.energyLevel}/5</Text>
          </View>
        )}
        {!item.validatorName && (
  ...
  return (
    <View style={[s.container, { backgroundColor: bg }]}>
      ...
      <View style={s.filtersRow}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.filtersContent} style={{ flex: 1 }}>
          {FILTERS.map(f => (
            <TouchableOpacity
              key={f.key}
              style={[s.filterChip, { borderColor: filterStatus === f.key ? green : border }, filterStatus === f.key && { backgroundColor: green }]}
              onPress={() => handleFilterChange(f.key)}
              activeOpacity={0.8}
            >
              <Text style={[s.filterChipText, { color: filterStatus === f.key ? '#ffffff' : textSecondary }]}>
                {f.label}
              </Text>
            </TouchableOpacity>
          ))}
            ...
            <View style={s.emptyWrap}>
              {searchQuery ? (
                <>
                  ...
                  <Text style={[s.emptyTitle, { color: textPrimary }]}>Sin resultados para "{searchQuery}"</Text>
                  <Text style={[s.emptySub, { color: textMuted }]}>Intenta con otro nombre</Text>
                  <TouchableOpacity style={[s.emptyOutlineBtn, { borderColor: green }]} onPress={() => handleSearch('')}>
                ...
                <>
                  ...
                  <Text style={[s.emptyTitle, { color: textPrimary }]}>
                    No hay alimentos {FILTERS.find(f => f.key === filterStatus)?.label.replace(/[✅⏳🔴]/g, '').trim().toLowerCase()}
                  </Text>
                  <TouchableOpacity style={[s.emptyOutlineBtn, { borderColor: green }]} onPress={() => handleFilterChange('todos')}>
                ...
                <>
                  ...
                  <TouchableOpacity style={[s.emptyFilledBtn, { backgroundColor: green }]} onPress={openCreate}>
          ...
          {selectedFood && (
            <SafeAreaView style={{ flex: 1 }}>
              <View style={s.heroWrap}>
                ...
                <View style={[s.heroBadge, { backgroundColor: selectedFood.validatorName ? '#f0fdf4' : '#fefce8' }]}>
                  <Text style={[s.heroBadgeText, { color: selectedFood.validatorName ? '#16a34a' : '#d97706' }]}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.detailScroll}>
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.detailChipsRow}>
                  ...
                  {!!selectedFood.energyLevel && (
                    <View style={[s.detailChip, { backgroundColor: ENERGY_COLORS[selectedFood.energyLevel] + '20', borderColor: ENERGY_COLORS[selectedFood.energyLevel] + '40' }]}>
                  ...
                  {selectedFood.validatorName && (
                    <View style={[s.detailChip, { backgroundColor: '#f0fdf4', borderColor: '#bbf7d0' }]}>
                      <Text style={[s.detailChipText, { color: '#16a34a' }]}>✅ {selectedFood.validatorName}</Text>
                    </View>
                  )}
                ...
                {detailTiming && (
                  <View style={[s.detailSection, { backgroundColor: cardBg, borderColor: border }]}>
                    ...
                    <Text style={[s.detailText, { color: textPrimary }]}>{detailTiming.icon} {detailTiming.text}</Text>
                  </View>
                )}
              </ScrollView>
              <View style={[s.detailFooter, { backgroundColor: cardBg, borderTopColor: border }]}>
                <TouchableOpacity style={[s.footerBtnMain, { backgroundColor: green }]} onPress={() => openEdit(selectedFood)} activeOpacity={0.85}>
                  <Text style={s.footerBtnText}>Editar</Text>
                </TouchableOpacity>
                {!selectedFood.validatorName && (
                  <TouchableOpacity style={[s.footerBtnSec, { backgroundColor: '#0284c7' }]} onPress={() => handleValidate(selectedFood)} activeOpacity={0.85}>
                    <Text style={s.footerBtnText}>Validar</Text>
                  </TouchableOpacity>
                )}
                <TouchableOpacity
                  style={[s.footerBtnToggle, { backgroundColor: selectedFood.isActive ? '#fef2f2' : '#f0fdf4', borderColor: selectedFood.isActive ? '#fca5a5' : '#bbf7d0' }]}
                  onPress={() => handleToggleActive(selectedFood)}
                  activeOpacity={0.85}
                >
                  <Text style={{ fontSize: 22 }}>{selectedFood.isActive ? '🔴' : '🟢'}</Text>
                </TouchableOpacity>
              </View>
            </SafeAreaView>
          )}
          ...
          <SafeAreaView style={{ flex: 1 }}>
            <View style={[s.formHeaderWrap, { backgroundColor: green }]}>
              <View style={s.formHeaderRow}>
                ...
                <Text style={s.formHeaderTitle}>{editingFood ? 'Editar alimento' : 'Nuevo alimento'}</Text>
                <View style={{ width: 44 }} />
              </View>
            </View>
            <ScrollView contentContainerStyle={s.formScroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              ...
              <View style={[s.sectionCard, { backgroundColor: cardBg, borderColor: border }]}>
                ...
                <TextInput
                  style={[s.input, { backgroundColor: inputBg, borderColor: nameError ? '#ef4444' : border, color: textPrimary }]}
                  value={formName}
                ...
                {formImageUrl.trim().length > 0 && (
                  <Image source={{ uri: formImageUrl.trim() }} style={s.imgPreview} resizeMode="cover" onError={() => { }} />
                )}
              </View>
              ...
              <View style={[s.sectionCard, { backgroundColor: cardBg, borderColor: border }]}>
                ...
                <View style={s.energySelector}>
                  {[1, 2, 3, 4, 5].map(n => (
                    <TouchableOpacity
                      key={n}
                      style={[s.energyBtn, { borderColor: ENERGY_COLORS[n] }, formEnergyLevel === String(n) && { backgroundColor: ENERGY_COLORS[n] }]}
                      onPress={() => setFormEnergyLevel(String(n))}
                    >
                      <Text style={[s.energyBtnText, { color: formEnergyLevel === String(n) ? '#fff' : ENERGY_COLORS[n] }]}>{n}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                <View style={s.energyLabelRow}>
                  {[1, 2, 3, 4, 5].map(n => (
                    <Text key={n} style={[s.energyLabelText, { color: formEnergyLevel === String(n) ? ENERGY_COLORS[n] : textMuted }]}>
                      {ENERGY_LABELS[n]}
                    </Text>
                  ))}
                </View>
            ...
            <View style={[s.formFooter, { backgroundColor: cardBg, borderTopColor: border }]}>
              <TouchableOpacity style={[s.formCancelBtn, { borderColor: border }]} onPress={() => setShowForm(false)} disabled={saving}>
                <Text style={[s.formCancelText, { color: textSecondary }]}>Cancelar</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[s.formSaveBtn, { backgroundColor: green, opacity: saving ? 0.7 : 1 }]} onPress={handleSave} disabled={saving}>
                {saving ? <ActivityIndicator color="#fff" size="small" /> : <Text style={s.formSaveText}>Guardar</Text>}
              </TouchableOpacity>
            </View>
          </SafeAreaView>
        </View>
...
const s = StyleSheet.create({
  ...
  searchWrap: {
    ...
    marginHorizontal: LIST_PAD, marginTop: 12, marginBottom: 4,
  ...
  filterChip: { borderRadius: BorderRadius.full, borderWidth: 1.5, paddingHorizontal: 14, paddingVertical: 7 },
  ...
  foodCard: {
    width: CARD_W, borderRadius: 16, overflow: 'hidden', borderWidth: 1,
  ...
  foodImgFallback: { width: '100%', height: 120, alignItems: 'center', justifyContent: 'center' },
  foodImgEmoji: { fontSize: 44 },
  validBadge: { position: 'absolute', top: 6, right: 6, backgroundColor: 'rgba(255,255,255,0.92)', borderRadius: 12, padding: 3 },
  validBadgeText: { fontSize: 12 },
  foodLabel: { minHeight: 44, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 8, paddingVertical: 7 },
  foodLabelText: { color: '#ffffff', fontSize: 11, fontWeight: '700', textAlign: 'center', lineHeight: 16 },
  cardChipsWrap: { flexDirection: 'row', flexWrap: 'wrap', gap: 4, padding: 7, paddingTop: 5, minHeight: 32 },
  cardChip: { borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 },
  ...
  heroOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, height: 140, backgroundColor: 'rgba(0,0,0,0.55)' },
  heroName: {
    position: 'absolute', bottom: 52, left: 16, right: 16,
    color: '#ffffff', fontSize: 20, fontWeight: '900',
    textShadowColor: 'rgba(0,0,0,0.5)', textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 4,
  },
  heroBack: { position: 'absolute', top: 14, left: 14, width: 42, height: 42, borderRadius: 21, backgroundColor: 'rgba(0,0,0,0.4)', alignItems: 'center', justifyContent: 'center' },
  heroBackText: { color: '#ffffff', fontSize: 20, fontWeight: '700' },
  heroBadge: { position: 'absolute', top: 14, right: 14, borderRadius: BorderRadius.full, paddingHorizontal: 12, paddingVertical: 6 },
  heroBadgeText: { fontSize: 12, fontWeight: '800' },
  detailScroll: { padding: 16, gap: 12, paddingBottom: 16 },
  detailChipsRow: { marginBottom: 4 },
  detailChip: { borderRadius: BorderRadius.full, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 6, marginRight: 8 },
  detailChipText: { fontSize: 12, fontWeight: '700' },
  detailDesc: { fontSize: 14, lineHeight: 22 },
  benefitsCard: { borderRadius: 14, borderLeftWidth: 3, borderWidth: 1, padding: 12, gap: 6 },
  detailSection: { borderRadius: 14, borderWidth: 1, padding: 12, gap: 6 },
  macroSection: { gap: 8 },
  detailSectionLabel: { fontSize: 11, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 0.5 },
  detailText: { fontSize: 13, lineHeight: 20 },
  ingredientItem: { fontSize: 13, lineHeight: 21 },
  macrosGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  macroPill: { width: '47%', borderRadius: 12, borderWidth: 1, padding: 12, alignItems: 'center', gap: 3 },
  macroIcon: { fontSize: 18 },
  macroVal: { fontSize: 15, fontWeight: '900' },
  macroLabel: { fontSize: 10, fontWeight: '600' },
  detailFooter: {
    flexDirection: 'row', gap: 10, alignItems: 'center',
    paddingHorizontal: 16, paddingVertical: 12, borderTopWidth: 1,
  },
  footerBtnMain: { flex: 2, borderRadius: BorderRadius.full, paddingVertical: 14, alignItems: 'center', justifyContent: 'center', elevation: 3 },
  footerBtnSec: { flex: 1, borderRadius: BorderRadius.full, paddingVertical: 14, alignItems: 'center', justifyContent: 'center' },
  footerBtnToggle: { width: 50, height: 50, borderRadius: 25, borderWidth: 1.5, alignItems: 'center', justifyContent: 'center' },
  footerBtnText: { color: '#ffffff', fontSize: 13, fontWeight: '800' },
  formHeaderWrap: { borderBottomLeftRadius: 24, borderBottomRightRadius: 24, paddingBottom: 14 },
  formHeaderRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingTop: 10 },
  formBackBtn: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' },
  formBackText: { color: '#ffffff', fontSize: 22, fontWeight: '700' },
  formHeaderTitle: { color: '#ffffff', fontSize: 17, fontWeight: '900' },
  formScroll: { padding: 16, gap: 4, paddingBottom: 16 },
  sectionHeader: { fontSize: 11, fontWeight: '800', letterSpacing: 0.8, textTransform: 'uppercase', marginTop: 14, marginBottom: 6 },
  sectionCard: { borderRadius: 16, borderWidth: 1, padding: 14, gap: 4 },
  fieldLabel: { fontSize: 12, fontWeight: '700', marginBottom: 4, marginTop: 6 },
  fieldLabelSm: { fontSize: 11, fontWeight: '700', marginBottom: 4 },
  input: { borderRadius: 12, paddingHorizontal: 14, paddingVertical: 11, fontSize: 14, borderWidth: 1.5 },
  inputSm: { borderRadius: 10, paddingHorizontal: 12, paddingVertical: 9, fontSize: 13, borderWidth: 1.5 },
  inputMulti: { borderRadius: 12, paddingHorizontal: 14, paddingVertical: 11, fontSize: 14, borderWidth: 1.5, textAlignVertical: 'top', minHeight: 76 },
  wordCounter: { fontSize: 11, fontWeight: '600', marginTop: 2 },
  imgPreview: { width: 72, height: 72, borderRadius: 10, marginTop: 8 },
  rowFields: { flexDirection: 'row', gap: 10 },
  quickToggle: { flex: 1, borderRadius: 12, borderWidth: 1.5, padding: 10, alignItems: 'center', justifyContent: 'center', gap: 6 },
  energySelector: { flexDirection: 'row', gap: 6 },
  energyBtn: { flex: 1, borderRadius: 10, borderWidth: 2, paddingVertical: 12, alignItems: 'center', justifyContent: 'center' },
  energyBtnText: { fontSize: 16, fontWeight: '900' },
  energyLabelRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 },
  energyLabelText: { flex: 1, fontSize: 9, fontWeight: '600', textAlign: 'center' },
  macrosForm: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  macroField: { width: '47%' },
  macroHint: { fontSize: 11, fontStyle: 'italic', marginTop: 4 },
  formFooter: { flexDirection: 'row', gap: 12, paddingHorizontal: 16, paddingVertical: 14, borderTopWidth: 1 },
  formCancelBtn: { flex: 1, borderRadius: BorderRadius.full, borderWidth: 1.5, paddingVertical: 14, alignItems: 'center' },
  formCancelText: { fontSize: 15, fontWeight: '700' },
  formSaveBtn: { flex: 2, borderRadius: BorderRadius.full, paddingVertical: 14, alignItems: 'center', elevation: 4, shadowColor: '#000', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.2, shadowRadius: 8 },
